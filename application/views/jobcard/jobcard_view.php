<div class="w-full bg-white rounded-2xl shadow-md p-8">
	...
	<div class="grid grid-cols-2 gap-6 border p-5 bg-gray-50 rounded-xl mb-6">
		...
		<div>
			...
			<p><strong>Technician:</strong> <?= $jobcard->technician ?></p>
		</div>
		...
		<tbody>
			...
			?>
				<tr class="border-b">
					...
					<td class="p-3">₹<?= number_format($p->rate, 2) ?></td>
	...
	<div class="border-t pt-4 text-right">
		<p class="text-lg"><strong>Service Total:</strong> ₹<?= number_format($service_total, 2) ?></p>
		<p class="text-lg"><strong>Parts Total:</strong> ₹<?= number_format($parts_total, 2) ?></p>
		<p class="text-xl font-bold mt-2"><strong>Grand Total:</strong> ₹<?= number_format($service_total + $parts_total, 2) ?></p>
	</div>
