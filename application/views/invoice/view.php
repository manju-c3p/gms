	...
	<div class="flex justify-between items-center mb-6">
		...
		<div class="flex items-center gap-2">
			...
			<a href="<?= base_url('index.php/Invoice'); ?>"
				class="p-3 rounded-lg bg-gray-200 hover:bg-gray-300 transition"
				title="Back to List">
			...
			<tbody>
				<?php if (!empty($payments)): ?>
					<?php foreach ($payments as $p): ?>
						<tr>
							<td class="border p-2"><?= date('d-m-Y', strtotime($p->voucher_date)) ?></td>
							<td class="border p-2"><?= $p->payment_type ?></td>
							<td class="border p-2 text-right">
								<?= number_format($p->amount, 2) ?>
							</td>
							<td class="border p-2"><?= $p->voucher_code ?></td>
							<td class="border p-2"><?= $p->narration ?></td>
						</tr>
					<?php endforeach; ?>
				<?php else: ?>
					<tr>
