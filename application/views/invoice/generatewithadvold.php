	...
	<form method="post" action="<?= base_url('index.php/Invoice/save') ?>" id="invoiceForm" class="hidden">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
			...
			<div class="bg-gray-50 p-4 rounded-lg border">
				...
				<div id="available_advance_box" class="mt-4 hidden">

					<h4 class="text-sm font-semibold text-gray-600 mb-2">
						Available Advance
					</h4>

					<div id="advance_list" class="space-y-2"></div>
	...
	function loadJobcardDetails(jobcardId) {
		...
		fetch(BASE_URL + 'Invoice/get_jobcard_details/' + jobcardId)
			...
			.then(data => {
				...
				fetch(BASE_URL + 'Advance/get_available_advance_custid/' + data.customer_id)
					.then(res => res.json())
					.then(advances => {
						...
						if (advances.length > 0) {
							...
							document.getElementById('available_advance_box').classList.remove('hidden');
							...
							advances.forEach(a => {
								html += `
									<div class="flex justify-between items-center border p-2 rounded">
										...
										<div>
											AED ${parseFloat(a.balance_amount).toFixed(2)}
										</div>

										<button type="button"
											onclick="applyAdvance(${a.advance_id}, ${a.balance_amount})"
							...
							html = `<p class="text-gray-400 text-sm">No advance available</p>`;
						}

						document.getElementById('advance_list').innerHTML = html;
					});
...
<script>
	...
	function applyAdvance(id, amount) {
    ...
    document.getElementById('advance_paid').value = useAmount.toFixed(2);

    // trigger calculation
    document.getElementById('advance_paid').dispatchEvent(new Event('input'));
}
</script>
