...
header("Content-Disposition:attachment;filename=export_quotation_report.xls");
...
<body>
	<table width="100%" border=0 cellspacing="0" colspacing="0">
		<tr>
			<td align="center">
				<p style="font-size:16px; font-weight:bold;">Print Quotation report</p>
			</td>
			...
			foreach ($records as $row) : ?>
				<tr>
					...
					<td><?php echo $row->quotation_code; ?>
						...
						if ($row->revision > 0) {
							for ($k = 1; $k <= $ev; $k++) { ?>
								<u><a target='_blank' href="<?php echo base_url() . 'index.php/Sales/print_quotation/' . $row->quote_id . '/' . $k . '/0'; ?>" title="View Revision">Revision <?php echo $k; ?></a></u><br>
						<?php }
						}
						?>
					</td>
