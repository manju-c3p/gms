<?php
$from_date = $this->uri->segment(4) ?? date('d-m-Y');
$to_date   = $this->uri->segment(5) ?? date('d-m-Y');
$request_type = $_GET['request_type'] ?? '';


// convert to d-m-Y display format if needed
$from_date_obj = DateTime::createFromFormat('Y-m-d', $from_date) ?: DateTime::createFromFormat('d-m-Y', $from_date);
$from_display = $from_date_obj ? $from_date_obj->format('d-m-Y') : date('d-m-Y');
$to_date_obj = DateTime::createFromFormat('Y-m-d', $to_date) ?: DateTime::createFromFormat('d-m-Y', $to_date);
$to_display = $to_date_obj ? $to_date_obj->format('d-m-Y') : date('d-m-Y');
?>
<div class="card-body">
    <form class="form-horizontal" action="<?php echo base_url(); ?>" id="receipt" method="post" name="receipt">
       ...
       value="<?php echo $from_display; ?>" required disabled>
                </div>
            </div>
            ...
            <div class="col-lg-3">
                <div class="input-group">
                   <input type="text" class="form-control form-control-sm datepicker1" name="from" id="from"
       value="<?php echo $to_display; ?>" required disabled>
                </div>
            </div>
            <label class="col-lg-1 col-form-label">Type</label>
            <div class="col-lg-3">
                <?php $request_type = $request_type ?? ''; ?>
    ...
    <?php if (!empty($records)) : ?>
        <input type="text" class="form-control" name="party_name" value="<?php echo $records[0]->party_name; ?>" readonly style="font-weight: bold;">
    <?php endif; ?>
</div>
        </div>
