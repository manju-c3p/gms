...
<?php foreach ($jobcardProgress1 as $row):

    $percent = ($row->total_jobs > 0)
        ? round($row->total_progress / $row->total_jobs)
        : 0;
?>
...
<div class="bg-white rounded-2xl shadow p-6">
	<h3 class="text-lg font-bold mb-4 flex items-center gap-2">
		<!-- 📊 Jobcard Work Progress -->
	</h3>

	<div class="space-y-4">
		<?php foreach ($jobcardReport as $r): ?>
			<div class="border rounded-xl p-4">

				<div class="flex justify-between text-sm mb-2">
					<div>
						<p class="font-semibold"><?= $r->jobcard_no ?></p>
					...
					<div class="text-right text-xs">
						<p><?= $r->worked_hours ?> hrs</p>
						<p class="font-semibold"><?= $r->progress ?>%</p>
					</div>
				</div>
				...
				<div class="w-full bg-gray-200 rounded-full h-2">
					<div
						...
						<?= $r->progress < 50 ? 'bg-yellow-500' : ($r->progress < 90 ? 'bg-blue-500' : 'bg-green-600') ?>"
						style="width: <?= $r->progress ?>%">
					</div>
				</div>

				<p class="text-xs text-gray-500 mt-1">
					<?= $r->completed_jobs ?> / <?= $r->total_jobs ?> jobs completed
				</p>
		...
		<?php foreach ($jobcardReport as $r): ?>
			...
			<div class="border rounded-xl p-4 hover:bg-gray-50 transition">
				...
				<div class="flex justify-between items-center mb-2">
					<div>
						<p class="font-semibold text-sm"><?= $r->jobcard_no ?></p>
					...
					<div class="text-right text-xs">
						<p class="font-medium"><?= $r->worked_hours ?> hrs</p>
						<p class="font-semibold"><?= $r->progress ?>%</p>
					</div>
				</div>
				...
				<div class="w-full bg-gray-200 rounded-full h-2">
					<div
						...
						<?= $r->progress < 40 ? 'bg-red-500' : ($r->progress < 80 ? 'bg-yellow-500' : 'bg-green-600') ?>"
						style="width: <?= $r->progress ?>%">
					</div>
				</div>
