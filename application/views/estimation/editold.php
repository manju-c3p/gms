...
<script>
	...
	function calculateGrandTotal() {
		...
		let taxPercent = num(document.getElementById("tax_percent")?.value);
		let discount = num(document.getElementById("discount")?.value);
	...
	function calculateSellingPrice(input) {
		...
		const rowTotalInput = row.querySelector(".rowTotal");
		const qtyInput = row.querySelector(".partQty");
		...
		const rowtot = sellingPrice * qty;

		sellPriceInput.value = sellingPrice.toFixed(2);

		rowTotalInput.value = rowtot.toFixed(2);
	}
</script>
