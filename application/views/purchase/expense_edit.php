	...
	<div class="bg-white shadow rounded-xl p-6 space-y-6">
		...
		<div class="grid grid-cols-12 gap-4">
			...
			<div class="col-span-12 md:col-span-4">
				...
				<select name="expense_ledger_id" id="expense_ledger_id"
					...
					<?php foreach ($expense_ledgers as $l) { ?>
						<option value="<?php echo $l->ledger_id; ?>" <?php if ($l->ledger_id == $expense->ledger_id) { ?> selected <?php } ?>>
			...
			<div class="col-span-12 md:col-span-3">
				...
				<div class="col-span-12 md:col-span-4 " id="bank_ledger_div">
					<label class="text-sm">Bank Account</label>
					<select name="bank_ledger_id" id="bank_ledger_id"
						class="w-full border rounded px-3 py-2 select2">
						<option value="">Select Bank</option>
						<?php foreach ($bank_ledgers as $l) { ?>
							<option value="<?php echo $l->ledger_id; ?>" <?php if ($l->ledger_id == $expense->bank_ledger_id) { ?> selected <?php } ?>>
								<?php echo $l->ledger_name; ?>
							</option>
						<?php } ?>
					</select>
...
<script>
    ...
    function paymentModeLogic() {
		
        var mode = $('#payment_mode').val();

        if (mode == 'CASH') {
            $('#bank_ledger_div').hide();
        } else {
            $('#bank_ledger_div').show();
        }
    }

    // Run on page load (DB loaded value)
    paymentModeLogic();
