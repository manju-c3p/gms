		...
		<div class="grid grid-cols-12 gap-4 items-end">
			...
			<div class="col-span-12 md:col-span-3">
				...
				<select class="w-full border rounded-lg px-3 py-2 select2"
					...
					<option value="Parts" <?php if ($po->purchase_type == 'Parts') echo 'selected'; ?>>Parts</option>
					<option value="Non-Parts" <?php if ($po->purchase_type == 'Non-Parts') echo 'selected'; ?>>Non Parts</option>
					...
					<tr>
						...
						<td class="p-2 border">
							<select class="w-full border rounded px-2 py-1 item-select2 select2 debtor-select"
								name="item_id[]" id="item<?php echo $i; ?>"
								onchange="get_item_by_id(<?php echo $i; ?>)">

								<option value="">Select</option>

								<?php foreach ($active_items as $it) { ?>
									<option value="<?php echo $it->part_id ?>"
										<?php if ($item->item_id == $it->part_id) echo 'selected'; ?>>
										<?php echo $it->part_name . ' (' . $it->part_type . ')'; ?>
									</option>
								<?php } ?>
						...
						<td class="p-2 border">
							<select class="w-full border rounded px-2 py-1"
								name="item_unit[]" id="unit<?php echo $i; ?>">
								<option value="">Select</option>
								<?php foreach ($active_units as $u) { ?>
									<option value="<?php echo $u->unit_id ?>"
										<?php if ($item->unit_id == $u->unit_id) echo 'selected'; ?>>
						...
						<td class="p-2 border">
							<input class="w-full border rounded px-2 py-1 text-right quantity"
								type="number" name="item_quantity[]"
						...
						<td class="p-2 border">
							<input type="number"
								...
								name="dis_per[]" step="any"
								id="discount_per<?php echo $i; ?>"
								value="<?php echo $item->dis_per; ?>">
						</td>

						<td class="p-2 border">
							<input type="number"
								class="w-full border rounded px-2 py-1 text-right dis_amt"
								name="dis_amt[]" step="any"
								id="discount_amt<?php echo $i; ?>"
								value="<?php echo $item->dis_amt; ?>">
						</td>

						<td class="p-2 border">
							<input type="number"
								class="w-full border rounded px-2 py-1 text-right total_price"
								name="total_price[]" step="any"
								id="total_price<?php echo $i; ?>"
								value="<?php echo $item->total; ?>">
						</td>

						<td class="p-2 border text-center">
							<a href="#" class="addRow text-green-600 text-lg"><i class="fa fa-plus-circle"></i></a>
							<a href="#" class="deleteRow text-red-600 text-lg"><i class="fa fa-trash"></i></a>
						</td>
	...
	<div id="service_table_section" class="bg-white shadow rounded-xl p-6 overflow-x-auto hidden">
		...
		<table id="service_table" class="w-full table-fixed text-sm border border-gray-300">
			...
			<tbody>
				...
				<?php $k = 1;
				foreach ($po_services as $srv) { ?>

					<tr>

						<td class="p-2 border text-center"><?php echo $k; ?></td>

						<td class="p-2 border">
							<input type="text" name="service_description[]"
								class="w-full border rounded px-2 py-1"
								value="<?php echo $srv->description; ?>">
						</td>
						...
						<td class="p-2 border">
							<input type="number" step="any"
								...
								value="<?php echo $srv->dis; ?>">
						</td>

						<td class="p-2 border">
							<input type="number" step="any"
								name="service_vat[]"
								class="service_vat w-full border rounded px-2 py-1 text-right"
								value="<?php echo $srv->vat; ?>">
						</td>
...
<script>
	...
	$(document).on('click', '.addServiceRow', function(e) {
        ...
        <td class="p-2 border">
            <input type="number" step="any" name="service_vat[]" class="service_vat w-full border rounded px-2 py-1 text-right">
        </td>
