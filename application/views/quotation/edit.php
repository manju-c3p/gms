	...
	<form method="post" action="<?= base_url('index.php/Quotation/update'); ?>" class="p-6 bg-white">
		...
		<div class="page-header flex items-center justify-between mb-4">
			...
			<div class="text-right mt-6">
				...
				<a href="<?= base_url('index.php/Quotation/view/'. $quotation->quotation_id ) ?>" class="ml-3 px-6 py-2 bg-gray-400 text-white rounded">
					View & Print
				</a>
				<a href="<?= base_url('index.php/Jobcard/edit_by_quotation/'. $quotation->quotation_id ) ?>" class="ml-3 px-6 py-2 bg-green-400 text-white rounded">
					Job Card
				</a>
			...
			<table class="w-full border-collapse text-sm">
				<tbody>
					...
					<tr>
						...
						<td class="border p-2">
							<input type="number" class="w-full border rounded px-2 py-1 bg-gray-100" value="<?= $kms ?>" readonly>
						</td>
				...
				<table class="w-full border-collapse text-sm" id="usedPartsTable">
					...
					<tbody>
						...
						<?php if (!empty($parts_used_used)): ?>
							<?php foreach ($parts_used_used as $i => $p): if ($p->selected ==1){ ?>
								<tr class="hover:bg-gray-50 transition">
									<td class="border px-2 py-2 text-center">
										<input type="checkbox"
											name="customer_selected[]"
